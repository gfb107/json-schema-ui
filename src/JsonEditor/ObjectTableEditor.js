...
export default class ObjectTableEditor extends JsonEditor {
  constructor( parms ) {
    ...
    if ( isObject ) {
      ...
      for ( let prop of Object.keys( propSchema.properties )) {
        ...
        this.setSchemaAttributes( cell, childSchema );
        row.appendChild( cell );
      }
    } else {
    ...
    if ( isObject ) {
      for ( let prop of props ) {
        ...
        for ( let childProp of Object.keys( childProps )) {
          ...
          this.setSchemaAttributes( cell, childProps[ childProp ]);
          row.appendChild( cell );
      ...
      for ( let prop of props ) {
        ...
        cell = this.createElement( 'td' );
        this.setSchemaAttributes( cell, propSchema );
        row.appendChild( cell );
  ...
  showErrors( errors ) {
    console.log( 'showErrors', this.path, errors );
    errors = errors || [];
    ...
    for ( let prop of Object.keys( childErrors )) {
      ...
      if ( child instanceof JsonEditor ) {
        ...
        console.log( 'child is not editor', child, errors );
        let subErrors = {};
        for ( let error of errors ) {
